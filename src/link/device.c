...
int searchAvaiDevice(const char *name, struct MacAddr *addr, uint32_t *ipAddr)
{
    ...
    for (pcap_if_t *p_dev = alldevs; p_dev; p_dev = p_dev->next)
    {
        ...
        for (pcap_addr_t *p_addr = p_dev->addresses; p_addr; p_addr = p_addr->next)
        {
            ...
            if (s_addr && s_addr->sa_family == AF_INET)
            {
                struct sockaddr_in *si_addr = (struct sockaddr_in *)s_addr;
                *ipAddr = si_addr->sin_addr.s_addr;
            }
        }
...
struct Device *addDevice(const char *deviceName)
{
    ...
    /* Search the device */
    if (searchAvaiDevice(deviceName, &macAddr, &ipAddr) == -1)
        return NULL;
    ...
    device->deviceDescriptor = ++(deviceList.maxcount);
    ...
    deviceList.tail->nextPointer = device;
    deviceList.tail = device;
    return device;
}
...
struct MacAddr getMacAddr(struct Device *device)
{
...
{
    return deviceList.head->nextPointer;
}